...
app.get('/user_main_loggedin', function(req,res) {
	conn.connect(function(err) {
		...
		conn.query(`
			...
			`, function (err, result) {
			if (err) throw err;
			res.render('user_main.ejs', {usermain:result});// shows landing page  and passes data from backend
		});
	});	
});
...
app.get('/myarticles', function(req,res) {
	conn.connect(function(err) {
		...
		conn.query(`
			...
			`, function (err, result) {
			if (err) throw err;
			res.render('admin/myarticles.ejs', {myarticles:result});// shows all articles written by admin
		});
	});	
});
...
app.get('/viewarticle', function(req,res) {
	...
	conn.query(`
		...
		`, function (err, result) {
		if (err) throw err;
	res.render('admin/viewarticle.ejs', {viewArticle:result});// shows read page 
});
});
});
